...
///     input_noise (double): mean value for input noise distribution
///     slip_fraction (double): mean value for slip fraction distribution
///     process_covariance (double): Q value for Q matrix in EKF filter
/// PUBLISHES:
///     (/odom) (Odometry): Body configuration and twist
///     (/blue/path): path taken by blue robot (uncorrected odometry)
///     (/green/path): path taken by green robot (SLAM EKF filter applied)
///     (/slam_map): map generated by SLAM EKF
/// SUBSCRIBES:
...
private:
  ...
  void js_callback(const sensor_msgs::msg::JointState & msg)
  {
    ...
    if (index == 100) {
      ...
      green_path_msg.poses.push_back(
        publish_path(
          quat_green, "green/base_footprint",
          extended_kalman.zeta_est(1), extended_kalman.zeta_est(2)));
      blue_path_msg.poses.push_back(
        publish_path(
          quat, "blue/base_footprint", tbot3.q.x,
          tbot3.q.y));
      index++;
    }
  ...
  geometry_msgs::msg::PoseStamped publish_path(
    geometry_msgs::msg::Quaternion quat,
    std::string frame_id, double x, double y)
  {
